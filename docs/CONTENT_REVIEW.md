...
node scripts/verify-site.mjs    # 56/56 ผ่าน — ตรวจว่าตัวเลขบนหน้าเว็บตรงกับไฟล์ข้อมูล
```
...
## 13. โมดูลที่เพิ่มในรอบที่ 3 (16 ก.ย. 2026) — ผลต่อความถูกต้องของเนื้อหา

รอบที่ 3 เพิ่มเนื้อหาใหม่ 4 ส่วน จึงต้องทบทวนว่าเนื้อหาใหม่ **สอดคล้องกับข้อเท็จจริง
ที่ทบทวนไว้ในหัวข้อ 1–12** และไม่ได้ทำให้ตัวเลขหรือสูตรเดิมเปลี่ยนความหมาย

### 13.1 สี่คำที่ต้องจำ (`site/index.html#four-words`)

ตัวเลขทุกตัวในส่วนนี้ยกมาจากผลการรันที่บันทึกไว้แล้ว ไม่มีตัวเลขใหม่ที่ไม่ได้มาจากข้อมูล

| ข้อความในเว็บ | ค่าจริง | แหล่งที่มาที่ตรวจได้ | สถานะ |
|---|---|---|---|
| ขอ 10 VU และ k6 รายงาน `vus`/`vus_max` = 10/10 ตลอด 12.36 วินาที | 10/10 · 12.36 s | `assets/data/metrics.json`, `docs/evidence/k6-csv-raw-sample.csv` | ตรง |
| 1,155 คำขอ ÷ 12.36 วินาที = 93.44 คำขอต่อวินาที | 93.44 | `metrics.json` (`http_reqs`) — และบทที่ 1 สอนสูตรเดียวกัน | ตรง |
| ในการรันนี้ 10 VU รวมกันได้ 93.44 คำขอต่อวินาที (ไม่ใช่ 10) | 93.44 จาก `http_reqs` ของการรันนี้ | `assets/data/metrics.json`, บทที่ 1 | ตรง |
| เฉลี่ย 54.25 ms แต่ p95 74.53 และ p99 466.29 ซึ่งสูงกว่าค่าเฉลี่ย 8.6 เท่า | 466.29 ÷ 54.25 = 8.595 → 8.6 | `assets/data/latency-samples.json` (บทที่ 4 ใช้ค่า 8.6 อยู่แล้ว) | ตรง |
| เกณฑ์ของสคริปต์ load คือ `p(95)<500`, `p(99)<1500`, `http_req_failed rate<0.01` | ตรงตามไฟล์ | `k6/load.js` (และ `site/examples/load.js` ซึ่งต้องตรงกันทุกไบต์) | ตรง |
| ถอด think time ออกแล้วได้เกือบเท่าตัว: 9.59 → 18.43 ที่ Rt 54.25 ms | 1000 ÷ 104.25 = 9.5923 → 9.59 และ 1000 ÷ 54.25 = 18.4331 → 18.43 | ตาราง "กำลังผลิตต่อ VU เมื่อ Rt = 54.25 ms" ในบทที่ 2 (แถว Tt = 50 และ Tt = 0) | ตรง |

**ข้อผิดพลาดที่พบและแก้ระหว่างรอบนี้:** ฉบับร่างแรกของส่วน "สี่คำที่ต้องจำ" เขียนว่า
"ที่ Rt 54.25 ms และ think time 50 ms หนึ่ง VU ยิงได้ 9.49 คำขอต่อวินาที" ซึ่ง**ไม่ตรง**
เพราะ 9.49 คือค่าของแถว **Tt = 1000 ms** (และเป็นยอดของ 10 VU ไม่ใช่ต่อ VU)
ค่าที่ถูกของแถว Tt = 50 ms คือ **9.59** (1000 ÷ 104.25) ตามตารางในบทที่ 2
จึงแก้ข้อความสองแห่ง (ใน `index.html` และ `workflow.html`) ให้ตรงกับตารางต้นทาง
และถอยข้อความใน `index.html` ไปใช้ค่าที่ตรวจได้ตรงที่สุดของบริบทนั้น คือ 10 VU รวมกันได้ 93.44
คำขอต่อวินาที ไม่ใช่ 10

### 13.2 กายวิภาคของสคริปต์ k6 (`site/examples.html#anatomy`)

- โค้ดที่แสดงคือ `examples/load.js` ไฟล์จริง — **ถอดเครื่องหมาย ①–⑨ ออกแล้วตรงกับไฟล์ทุกไบต์**
  (ตรวจด้วย `browser-check.mjs` ส่วนที่ 9: 3,522 อักขระเท่ากัน) จึงไม่มีทางที่โค้ดบนหน้า
  จะเป็นเวอร์ชันที่รันไม่ได้
- หมายเลขทั้ง 9 ตรวจ anchor กับบรรทัดจริงในไฟล์ทุกตัว ถ้าบรรทัดเลื่อนจะฟ้องทันที
- คำอธิบายทั้ง 9 ข้อสอดคล้องกับหัวข้อที่ทบทวนไว้แล้ว:
  · `check` ≠ threshold → หัวข้อ 7 (checks) และบทที่ 3
  · `sleep` = think time ที่ถูกนับใน `iteration_duration` → หัวข้อ 1 และบทที่ 1
  · options = load model + stages → หัวข้อ 5 (scenarios/executors) และบทที่ 5
  · `abortOnFail` + `delayAbortEval` เลื่อนเฉพาะเวลาตัดสินใจหยุด ไม่ได้ตัดข้อมูล
  ramp-up ออกจากการคิด p95 → **หัวข้อ 9.1** ซึ่งเป็นข้อที่แก้ในรอบก่อน จึงเขียนให้ตรงกัน
  · custom metric (`Trend`/`Rate`) → หัวข้อ 6 และบทที่ 3
  · `ALLOWED_HOSTS` → กลไกที่มีอยู่จริงในสคริปต์ทั้งห้า (หัวข้อ 4)

### 13.3 หน้างานจริง (`site/workflow.html`)

- **ไม่ซ้ำกับ `troubleshooting.html`** — หน้านี้พูดถึงลำดับการทำงานของคน
  ส่วน `troubleshooting.html` พูดถึงอาการของเครื่องมือ (ตรวจแล้วว่าไม่มีหัวข้อซ้ำกัน)
- ความผิดพลาด 3 ข้อที่อธิบาย ใช้ตัวเลขที่ตรวจแล้วทั้งสิ้น
  (9.49 และ 18.43 ต่อ VU · 1.94 เท่า · `ALLOWED_HOSTS`) และ **ไม่ได้อ้างว่าเป็นอาการที่พบจาก
  การรันของโปรเจกต์นี้** — เขียนเป็นความผิดพลาดที่พบบ่อยทั่วไป ซึ่งถูกต้องในฐานะคำเตือน
- เช็คลิสต์ 8 ข้อเป็นข้อปฏิบัติ ไม่มีข้อใดอ้างตัวเลขหรือผลการวัด จึงไม่มีอะไรต้องตรวจกับข้อมูล
- **ไม่นับเป็นบทเรียนที่ 7** — ไม่มีเลขบท ไม่เข้า `LESSON_ORDER`
  และมีด่านตรวจยืนยันว่าจำนวนบทเรียนยังเป็น 6 และแบบทดสอบยัง 6 ชุด/36 ข้อ

### 13.4 สไลเดอร์ในเครื่องคำนวณ (`site/tools/calculator.html`)

- **`assets/js/calc.js` ไม่ถูกแก้เลย** — ชุดทดสอบ 95 เคสผ่านเท่าเดิม
  ตัวเลขทุกตัวในผลลัพธ์จึงยังคำนวณด้วยสูตรเดิม
- ช่วงค่าของสไลเดอร์เป็นเพียงเพดานเพื่อการสำรวจ ไม่ใช่ข้อจำกัดของสูตร
  ค่าที่พิมพ์เกินช่วงสไลเดอร์ยังคำนวณตามปกติ (ระบุไว้ในหมายเหตุใต้สูตรในหน้าแล้ว)
- ค่าที่พิมพ์ไว้ **ไม่ถูกสไลเดอร์แก้** แม้อยู่นอกช่วง และข้อความเตือนเมื่อค่าเป็น 0/ติดลบ
  ทำงานเหมือนเดิม — ยืนยันด้วยการทดสอบจริงใน `browser-check.mjs` ส่วนที่ 7.5

### 13.5 สิ่งที่ *ไม่ได้* ยืนยันในรอบที่ 3

- **ไม่ได้ยืนยันตัวเลข k6 ใหม่** — รอบนี้ไม่แตะ `k6/` และ `mock-server/` และไม่ได้รัน k6 เลย
  ตัวเลขทุกตัวยังเป็นหลักฐานจากรอบก่อนหน้า ไม่ได้รันซ้ำ
- **ไม่ได้ทดสอบกับโปรแกรมอ่านหน้าจอจริง** — ตรวจเฉพาะโครงสร้าง (label ผูกครบ,
  `aria-valuetext`, `role="progressbar"`, `aria-live`) ตามหลักเกณฑ์ ไม่ได้ฟังเสียงจริง
- **ไม่ได้ทดสอบบน Safari/Firefox** — เฉพาะ Chromium
- **คอนทราสต์หัวเลื่อนสไลเดอร์วัดจากค่าโทเคนที่เบราว์เซอร์ resolve แล้ว** ไม่ใช่การสุ่มพิกเซล
  เพราะ Chromium ไม่คืนสไตล์ของ `::-webkit-slider-thumb` ผ่าน `getComputedStyle`
- **เกณฑ์จำนวนคำรอบนี้ไม่ใช่เกณฑ์ "ตัดสำนวน"** — เปลี่ยนเป็น "ไม่คลาดจากค่าอ้างอิง ±5%/10%"
  เพราะรอบนี้เป็นการเพิ่มโมดูลโดยเจตนา ค่าที่ลดจากก่อนตัดสำนวนยังอยู่ที่ 26.5% (ไม่ใช่ 26.9%
  ของรอบที่ 1) โดย 66 คำที่เพิ่มขึ้นทั้งหมดคือบรรทัดอ้างกลับในบทเรียน 6 บรรทัด
